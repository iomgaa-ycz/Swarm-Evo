...
Quality  = 0.7 * node_pheromone + 0.3 * gene_pheromone
...
# """Deterministic per-locus gene selection with optional DPP reranking."""
...
# from core.execution.journal import Journal, Node
# from core.evolution.embedding_manager import CodeEmbeddingManager
# from core.evolution.faiss_similarity import all_pairs_cosine
# from core.evolution.gene_registry import (
#     GeneRegistry,
#     compute_gene_id,
#     normalize_gene_text,
...
# #     created_step: int
# #     quality: float = 0.0
# # normalized，用于 embedding / 相似度 / gene_id
# # 原始代码，用于 merge
# @dataclass
# class GeneItem:
#     locus: str
...
#     dpp_k: int = 1,
# ) -> Dict[str, Any]:
#     """Select a merge-compatible gene plan using pheromone-driven pools."""
#     pools = build_decision_gene_pools(journal, gene_registry)
...
#         if not items:
#             raise ValueError(f"Missing locus: {locus}")
#         winner = _select_locus_winner(items, use_dpp=use_dpp, dpp_k=dpp_k)
#         winners[locus] = winner
#         log_msg(
#             "INFO",
#             f"[WINNER] {locus}: "
#             f"gene={winner.gene_id[:6]} "
#             f"q={winner.quality:.4f} "
#             f"node={winner.source_node_id[:6]}"
#         )

#     gene_plan: Dict[str, str] = {
#         "reasoning": (
#             "Per-locus selection: Top-1 after DPP re-ranking (q=pheromone, sim=cosine). "
#             "Deterministic."
#             if use_dpp
#             else "Per-locus selection: Top-1 by pheromone score. Deterministic."
#         )
#     }
#     for locus, item in winners.items():
#         field_name = LOCUS_TO_FIELD[locus]
...
# #gene_pool子集筛选，只收合法节点（review且无buggy）
# def build_decision_gene_pools(
#     journal: Journal,
#     gene_registry: GeneRegistry,
# ) -> Dict[str, List[GeneItem]]:
...
#             node_pheromone = 0.0
#             if node.metadata:
#                 node_pheromone = float(node.metadata["pheromone_node"])
#             item = GeneItem(
#                 locus=locus,
#                 gene_id=gene_id,
#                 content=normalized,
...
# def _compute_quality(item: GeneItem) -> float:
#     quality = QUALITY_BLEND * item.gene_pheromone + (1.0 - QUALITY_BLEND) * item.node_pheromone
#     return quality


# def _select_locus_winner(items: List[GeneItem], use_dpp: bool, dpp_k: int = 1) -> GeneItem:
#     if len(items) == 1:
#         log_msg("INFO", f"[DPP] locus={items[0].locus} n=1 k={dpp_k} use_dpp={use_dpp}")
#         log_msg("INFO", "[DPP] selected_indices=[0]")
#         return items[0]
#     sorted_items = sorted(
#         items,
#         key=lambda gi: (-gi.quality, -gi.node_pheromone, -gi.source_score, gi.source_node_id, gi.gene_id),
#     )
#     if len(sorted_items) < 3 or not use_dpp:
#         log_msg(
#             "INFO",
#             f"[DPP] locus={items[0].locus} n={len(items)} k={dpp_k} use_dpp={use_dpp}",
#         )
#         log_msg("INFO", "[DPP] selected_indices=[0]")
#         return sorted_items[0]

#     try:
#         embeddings = embedding_manager.embed_texts([item.content for item in sorted_items])
#         similarity = all_pairs_cosine(embeddings)
#         q = np.array([item.quality for item in sorted_items], dtype=np.float32)
#         kernel = (q[:, None] * q[None, :]) * similarity
#         kernel += EPS * np.eye(len(sorted_items), dtype=np.float32)

#         log_msg(
#             "INFO",
#             f"[DPP] locus={sorted_items[0].locus} n={len(sorted_items)} k={dpp_k} use_dpp={use_dpp}",
#         )

#         dpp = FiniteDPP("likelihood", L=kernel)
#         dpp.rng = np.random.default_rng(0)
...
#                 key=lambda idx: (
#                     diag[idx],
#                     sorted_items[idx].quality,
#                     sorted_items[idx].node_pheromone,
#                     sorted_items[idx].source_score,
#                     sorted_items[idx].source_node_id,
#                     sorted_items[idx].gene_id,
#                 ),
#             )
#             selected_indices = [best_idx]
#         else:
#             size = min(dpp_k, len(sorted_items))
#             selected_indices = dpp.sample_exact_k_dpp(size=size)
#             if not selected_indices:
#                 selected_indices = [0]
#             selected_indices = sorted(selected_indices)

#         log_msg("INFO", f"[DPP] selected_indices={selected_indices}")
#         return sorted_items[selected_indices[0]]
#     except Exception:
#         log_msg("WARNING", f"[DPP] Falling back to quality sort for locus={sorted_items[0].locus}")
#         log_msg("INFO", "[DPP] selected_indices=[0]")
#         return sorted_items[0]
